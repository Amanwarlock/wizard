db.stockledgers.aggregate([{
	...
	$project: {
		...
		"snapShotId": 1,
		"mrp": 1,
		"qty": {$add : ["$stockTransaction.quantity" , "$stockTransaction.onHold"]},
		"ledgerGrn": 1,
		"referenceType": 1,
		"warehouseData": {
			"$arrayElemAt": ["$warehouseData", 0]
		},
		"createdAt": 1,
	}
}, {
	...
	"$project": {
		...
		"entry": {
			$filter: {
				...
				"cond": {
					$and: [{
						"$gt": ["$$this.ledgerGrn", null]
					}, {
						"$ne": ["$$this.referenceType", "Order Reservation"]
					}]
				}
			}
		},
		...
		"purchase_Price": {
			$cond: {
				...
				then: {
					$reduce: {
						input: {
							$arrayElemAt: ["$grnData.productDetails", 0]
						},
						initialValue: 0,
						in: {
							$sum: ["$$value", {
